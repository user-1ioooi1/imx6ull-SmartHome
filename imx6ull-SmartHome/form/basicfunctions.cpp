...
#include "library/cardstylebtn.h"
    ...
    ui(new Ui::basicFunctions)
{
    ...
    ui->beepBtn->setIconSource(":/icon/icon/门铃.png");
    ui->doorLockBtn->setIconSource(":/icon/icon/关锁.png");
    ui->ledBtn->setIconSource(":/icon/icon/灯亮.png");


    ui->ledBtn->setActiveColor(QColor("#FF6B35"));
    ui->beepBtn->setActiveColor(QColor("#5B7FFF"));
    ui->doorLockBtn->setActiveColor(QColor("#FF6B9D"));

    ui->ledBtn->setTitle("灯光");
    ui->beepBtn->setTitle("门铃");
    ui->doorLockBtn->setTitle("门锁");
    ...
    WeatherWidget *weather = new WeatherWidget(this);
    ui->weatherLayout->addWidget(weather);
}
...
void basicFunctions::onSensorDataUpdated(const SensorData &data)
{
    ...
    if(data.motionDetected == 1){
        ui->hasPersonLabelNum->setText("有人");
    }else{
        ui->hasPersonLabelNum->setText("无人");
    }
...
{
    SensorMonitor::instance()->stop();
}

void basicFunctions::openSensorTimer()
{
    SensorMonitor::instance()->start(2000); //2s
}

void basicFunctions::btnStateChange(CardStyleBtn *btn, bool on){
    if(on){
        btn->setChecked(on);
    }else{
        btn->setChecked(on);
